...
class ExampleInstrumentedTest {
    ...
    fun useAppContext() {
        ...
        assertEquals("com.github.TempTalkOrg.audio_pipeline.test", appContext.packageName)

        val processor = AudioPipelineProcessor()

        processor.initializeAudioProcessing(48000, 1)
        ...
        FileOutputStream(outputFile).use { outputStream1 ->
            FileOutputStream(outputFile2).use { outputStream2 ->
                while (pcmData.remaining() >= 960) {
                    ...
                    pcmData.get(chunk)
                    outputStream2.write(chunk)
                    processor.processAudio(1, 2, ByteBuffer.wrap(chunk))
