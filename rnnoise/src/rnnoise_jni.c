...
Java_com_github_TempTalkOrg_denoise_1filter_DenoisePluginAudioProcessor_create(JNIEnv *env, jobject thizl)
{
...
Java_com_github_TempTalkOrg_denoise_1filter_DenoisePluginAudioProcessor_destroy(JNIEnv *env, jobject thiz, jlong st)
{
    if (st != 0)
    {
...
Java_com_github_TempTalkOrg_denoise_1filter_DenoisePluginAudioProcessor_processFrame(JNIEnv *env, jobject thiz, jlong st, jbyteArray pcm)
{
    if (st == 0 || pcm == NULL)
    {
        return 0.0f;
